...
If your serial module is not '/dev/ttyUSB0', you should edit the script according to your serial device name, i.e. replace the port for the function `serial.Serial(port, baud)`. For Windows user, it seems like COM1, COM2, COM3 ... and for Linux, you can find them by `ls -l /dev | grep ttyUSB`
